...
$clearanceRequestQuery = $conn->prepare("SELECT * FROM `clearance_requests` WHERE user_id = ?");
$clearanceRequestQuery->execute([$_SESSION['user_id']]);
$clearanceRequests = $clearanceRequestQuery->fetchAll(PDO::FETCH_ASSOC);
?>
...
<body>
    ...
    <div class="list">
        ...
        <?php
        if (empty($clearanceRequests)) {
            echo "<p>No clearance requests found.</p>";
        } else {
            echo "<table>";
            echo "<thead><tr><th>Name</th><th>Purpose</th><th>Age</th><th>Address</th><th>Date Submitted</th><th>Status</th><th>Action</th></tr></thead>";
            echo "<tbody>";
            foreach ($clearanceRequests as $request) {
                ...
                echo "<td><a href='../certificate/cr_certificate.php?user_id={$request['user_id']}' target='_blank'>View</a></td>";
                echo "</tr>";
            }
            echo "</tbody>";
            echo "</table>";
