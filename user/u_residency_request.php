...
$checkRequestQuery = $conn->prepare("SELECT * FROM `residency_requests` WHERE user_id = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $insertQuery = $conn->prepare("INSERT INTO `residency_requests` (user_id, purpose, name, age, civil_status, citizenship, gender, address, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $insertQuery->execute([$_SESSION['user_id'], $purpose, $name, $age, $civil_status, $citizenship, $gender, $address, $date]);
...
<head>
    ...
    <link rel="stylesheet" href="../css/u_residency_request.css">
    <style>
        .modal {
            display: none;
            position: fixed;
...
<body>
    ...
    <div class="residency-form">
        <h2>Barangay Residency Request Form</h2>

        <form action="" method="post" onsubmit="return checkExistingRequest();">
